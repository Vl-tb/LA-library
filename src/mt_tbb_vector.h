...
////
//
#ifndef TEMPLATE_MT_TBB_VLAD_H
#define TEMPLATE_MT_TBB_VLAD_H
...
template<typename T, typename S, typename F> void mt_tbb_prod(const std::vector<T>& vector, const std::vector<S>& vc1,
                                      F& res) {
    res = tbb::parallel_reduce( tbb::blocked_range<int>(0, vector.size()), 0,
                                [&](tbb::blocked_range<int> r, double running_res){
                                    for (int i=r.begin(); i<r.end(); ++i) {
                                        running_res += static_cast<double>(vector[i])*static_cast<double>(vc1[i]);
                                    }
                                    return running_res;
                                }, std::plus<double>());
}
